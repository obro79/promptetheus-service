...
class SupabasePostgresStore:
    ...
    def ensure_workspace_project_for_user(
        ...
        *,
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                if membership_row is None:
                    ...
                    membership = _public_row(cur.fetchone())
                    created_workspace = True
                else:
                    workspace_id = str(membership_row["workspace_id"])
                    workspace_row = {
                        "id": workspace_id,
                        "name": membership_row["workspace_name"],
                        "created_at": membership_row["workspace_created_at"],
                    }
                    membership = _public_row(
                        {
                            "workspace_id": workspace_id,
                            "user_id": membership_row["user_id"],
                            "role": membership_row["role"],
                            "created_at": membership_row["member_created_at"],
                        }
                    )

                cur.execute(
                    """
                    SELECT id, workspace_id, name, api_key_hash, api_key_preview,
                           api_key_rotated_at, retention_days, created_at
                    FROM project
                    WHERE workspace_id = %s
                ...
                if project_row is None:
                    project_id = _next_id("proj")
                    cur.execute(
                        """
                        INSERT INTO project (
                          id, workspace_id, name, api_key_hash,
                          api_key_preview, api_key_rotated_at
                ...
                elif project_row.get("api_key_hash") is None:
                    cur.execute(
                        """
                        UPDATE project
                        SET api_key_hash = %s,
                        ...
                        RETURNING id, workspace_id, name, api_key_preview,
                                  api_key_rotated_at, retention_days, created_at
                        """,
                        (api_key_hash, api_key_preview, project_row["id"]),
                    )
                    project_row = cur.fetchone()
                    api_key_created = True
            conn.commit()
        ...
        return {
            "workspace": _public_row(workspace_row),
            "membership": membership,
            "project": self._project_row_to_dict(project_row),
    ...
    def create_agent(self, agent: dict[str, Any]) -> dict[str, Any]:
        row = {
            "id": str(agent.get("id") or _next_id("agent")),
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO agent (id, workspace_id, project_id, name)
                    VALUES (%(id)s, %(workspace_id)s, %(project_id)s, %(name)s)
                    RETURNING id, workspace_id, project_id, name, created_at
                    """,
                    row,
                )
                created = cur.fetchone()
            conn.commit()
        return _public_row(created)
